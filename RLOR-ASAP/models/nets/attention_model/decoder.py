...
class Decoder(nn.Module):
    ...
    def advance(self, cached_embeddings, state, urgency_factor):
        ...
        logits, glimpse = self.calc_logits(query, glimpse_K, glimpse_V, logit_K, mask, urgency_factor)

        return logits, glimpse

    def calc_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, urgency_factor):
        ...
        filtered_urgency = torch.tensor(urgency_factor, dtype=torch.float32, device=query.device).clone()  # Make a copy to avoid modifying the original
        filtered_urgency[mask] = 0
        #print("FILTERED URGENCY")
        #print(filtered_urgency)
        logits = self.pointer(glimpse, logit_K, mask)
        #print("INIT LOGITS")
        #print(logits)
        # if torch.isnan(logits).any():
        #     print("⚠️ NaNs detected in logits before Categorical. Fixing...")
        #     logits = torch.nan_to_num(logits, nan=-10.0)
        # logits = torch.tanh(torch.clamp(logits, min=-10, max=10))
        # if torch.isnan(filtered_urgency).any() or torch.isinf(filtered_urgency).any():
        #     print("⚠️ NaNs/Infs detected in `filtered_urgency`. Fixing...")
        #     filtered_urgency = torch.nan_to_num(filtered_urgency, nan=0.0, posinf=10.0, neginf=-10.0) 
        updated_logits = torch.tanh(logits.detach().clone()) 
        updated_logits[:, :, :] = updated_logits[:, :, :] + filtered_urgency[:, :, :] # Cuz depot doesn't have time window constraints
        updated_logits = updated_logits * 10
        # if torch.isnan(updated_logits).any():
        #     print("⚠️ NaNs detected after adding urgency factor! Fixing...")
        #     updated_logits = torch.nan_to_num(updated_logits, nan=-10.0)
        #print("(tanh(LOGITS) + urgency)*10")
        #print(updated_logits)
        return logits, glimpse
